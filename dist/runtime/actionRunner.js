...
import { cloneManagementMenuOptions, managementMenuStorageKey, readStoredManagementMenuOptions, readStoredRuntimeUiPreferences, runtimeUiStorageKey, } from "./runtimePreferences.js";
function getRuntimeUi(target) {
    return document.querySelector(`[data-runtime-ui="${target}"]`);
}
function getManagementMenuTargets(elements) {
    return {
        balloon: elements.balloonActionMenu,
        panel: elements.panelActionMenu,
    };
}
export function createActionRunner(context) {
    const { elements, state, dialogue, pluginRegistry, storageAdapter, actionTimers, managementMenu, eventBus, renderSpeech, renderPreviewSpeech, renderCharacterState, setLayerAnimationActive, addLog, touchInteraction, } = context;
    ...
    const runtimeUiPreferences = {};
    const managementMenuOptionsReady = loadManagementMenuOptions();
    const runtimeUiPreferencesReady = loadRuntimeUiPreferences();
    let pendingMenuPreviewItemId = null;
    let lastPreviewedMenuItemId = null;
    let menuPreviewTimerId = null;
    async function loadManagementMenuOptions() {
        try {
            const storedOptions = readStoredManagementMenuOptions(await storageAdapter.get(managementMenuStorageKey));
    ...
    function applyBalloonTheme(theme) {
    ...
    function applyBalloonFontSize(size) {
    ...
    function applyCharacterPosition(position) {
        if (!position) {
            return;
        }
    ...
    async function loadRuntimeUiPreferences() {
        try {
            const storedPreferences = readStoredRuntimeUiPreferences(await storageAdapter.get(runtimeUiStorageKey));
    ...
    function previewManagementMenuItem(item) {
        if (!item.description) {
            return;
        }
    ...
    registerAction("emit_event", (action, _) => {
        const a = action;
        eventBus.emit(a.event, a.payload);
    });
