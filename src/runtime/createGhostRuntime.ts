...
const defaultTiming: RuntimeTimingOptions = {
  idleDelay: 18000,
...
const interactiveAreaIds = ["runtimeTitle", "eventLog", "commandMenu"] satisfies InteractiveAreaId[];
const commandIds = ["fortune", "line", "hide"] satisfies RuntimeCommandId[];
 ...
 */
function createDefaultRules(timing: RuntimeTimingOptions): RuntimeRule[] {
  return [
    {
      id: "runtime.ready",
      event: "runtime:ready",
...
export function createGhostRuntime(options: GhostRuntimeOptions): GhostRuntime {
  ...
  const elements = {
    stage: requiredElement<HTMLElement>(options.selectors.stage),
    sprite: requiredElement<HTMLButtonElement>(options.selectors.sprite),
    spriteImage: requiredElement<HTMLImageElement>(options.selectors.spriteImage),
    speakerName: requiredElement<HTMLSpanElement>(options.selectors.speakerName),
    speechText: requiredElement<HTMLParagraphElement>(options.selectors.speechText),
    balloonActionMenu: optionalElement<HTMLElement>(options.selectors.balloonActionMenu),
    eventLog: requiredElement<HTMLOListElement>(options.selectors.eventLog),
    menuButtons: document.querySelectorAll<HTMLButtonElement>(options.selectors.menuButtons),
    observeAreas: document.querySelectorAll<HTMLElement>(options.selectors.observeAreas),
    statusMode: optionalElement<HTMLElement>(options.selectors.statusMode),
    statusExpression: optionalElement<HTMLElement>(options.selectors.statusExpression),
    statusVisibility: optionalElement<HTMLElement>(options.selectors.statusVisibility),
    statusLastEvent: optionalElement<HTMLElement>(options.selectors.statusLastEvent),
    statusIdleCountdown: optionalElement<HTMLElement>(options.selectors.statusIdleCountdown),
    statusRandomPrompt: optionalElement<HTMLElement>(options.selectors.statusRandomPrompt),
    statusActionTimers: optionalElement<HTMLElement>(options.selectors.statusActionTimers),
  };
  ...
  function renderManagementMenu(
    action: Extract<RuntimeAction, { type: "open_management_menu" }>,
    currentItems = action.items,
    parentItems?: typeof action.items,
   ...
   */
  function runActions(actions: RuntimeAction[]) {
   ...
   */
  function passesConditions(conditions: RuntimeCondition[] = []) {
    const now = Date.now();
