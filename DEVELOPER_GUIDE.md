...
개발자가 일반적인 기능 연결을 할 때 우선 보는 파일은 3개다.
...
현재 `src/ghost` 파일들은 기본 동작을 보여주기 위해 `src/demo` preset을 가져다 쓴다.

서비스에 붙일 때는 `src/demo`를 직접 고치기보다, `src/ghost/character.ts`, `src/ghost/nanika.config.ts`, `src/ghost/actions.ts`에서 필요한 preset import를 교체하거나 자기 기능을 추가하는 흐름을 우선한다.
...
| `featureOptions` | hover 설명, hitbox debug 같은 기능 토글 |
