...
GhostNest의 설정은 개발자 제어와 사용자 설정을 분리한다.

개발자는 나니카가 어떤 기능을 쓸 수 있는지, 어떤 에셋과 플러그인을 연결할지, 어떤 이벤트를 어떤 액션에 매핑할지 결정한다. 사용자는 개발자가 허용한 범위 안에서 말풍선 테마, 대사창 배치, 글자 크기 같은 개인 UI 취향만 바꾼다.

따라서 런타임 제어는 다음 4개 축으로 정리한다.
...
Capability / Plugin
-> 나니카가 어떤 외부 기능을 사용할지 정한다.

Action
-> 나니카가 실제로 수행할 행동 단위를 정한다.

Mapping / Scenario
-> 어떤 상황에서 어떤 액션과 기능을 연결할지 정한다.
```

이 기준에서 `controls`는 기능의 코어 구현을 바꾸는 옵션이 아니다. 흩어진 기능 진입점을 한 곳에서 켜고 끄는 개발자용 관문이다.

예를 들어 `controls.managementMenu`가 꺼져 있으면 메뉴 구성 데이터가 있어도 관리 메뉴 액션은 실행되지 않는다. `controls.randomPrompt`가 꺼져 있으면 random prompt 시간값이 있어도 이벤트가 발생하지 않는다.

세부 설정은 계속 각자의 책임을 유지한다.

- `controls`: 개발자가 이 런타임에서 무엇을 허용할지 정한다.
- `timing`: 켜진 기능의 시간값을 정한다.
- `typing`: 타이핑 연출 방식을 정한다.
- `speechLayout`: 대사 UI 표현 방식을 정한다.
- `managementMenu`: 메뉴를 켰을 때 어떤 메뉴와 표시 방식을 쓸지 정한다.
- `devtools`: 개발 도구를 켰을 때 어떤 DOM에 연결할지 정한다.
- `storageAdapter`: 저장이 허용될 때 어디에 저장할지 정한다.

최종적으로 개발자 도구는 캐릭터, 기능/플러그인, 액션, 매핑을 화면에 노출하고, 개발자는 연결만으로 나니카 동작을 구성할 수 있어야 한다.

코드에서 이 4개 축을 실제 실행 단위로 묶을 때는 `src/plugins/nanikaMapping`의 `NanikaRuntimePreset`을 사용한다. 프리셋은 캐릭터, 플러그인, rule/action 매핑, 런타임 옵션을 하나의 boot 가능한 단위로 모으되, 각 원본 파일의 책임을 강제로 합치지는 않는다.

`NanikaRuntimePreset`, 액션 카탈로그, 이벤트 카탈로그, 기능 카탈로그는 런타임 코어가 아니라 개발자용 조립 플러그인의 책임이다. 런타임 코어는 최종 `GhostRuntimeOptions`, `RuntimeRule`, `RuntimeAction`을 실행하는 데 집중한다.

개발자 도구 UI는 `src/plugins/nanikaMapping/registry.ts`를 통해 캐릭터, 기능, 이벤트, 액션, 매핑 목록을 가져온다. 각 목록의 원천 파일은 분리하되, 화면에서 참조하는 입구는 registry로 모아 유지한다.
